...
async function main() {
    ...
    if (!fs.existsSync('web/public/assets/brute_all')) fs.mkdirSync('web/public/assets/brute_all', { recursive: true });

    // Generate 200 thumbnails
    for (let w = 1; w <= 200; w++) {
        const image = new Jimp(w, 50, 0x00000000);
        let ptr = dataStart;
        let x = 0, y = 0;
        while (ptr < end && y < 50) {
            const pix = data[ptr++];
        ...
        await image.writeAsync(`web/public/assets/brute_all/w_${w}.png`);
    }
    console.log("Generated 200 brute width tests.");
}
