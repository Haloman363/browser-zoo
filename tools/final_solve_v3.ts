...
async function main() {
    ...
    // Palette: skip 4-byte header, then 256 * 4 bytes (BGRA)
    const palette: number[][] = [];
    for (let i = 0; i < 256; i++) {
        const idx = 4 + i * 4;
        const b = palData[idx];
        const g = palData[idx + 1];
        const r = palData[idx + 2];
        const a = palData[idx + 3];
    ...
    await img.writeAsync('web/public/assets/test/buffalo_v3.png');
    console.log("Saved buffalo_v3.png");
}
